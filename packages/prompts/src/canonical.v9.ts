/**
 * Prompt canónico v9 (spec v2, 6.3). Las reglas 17 y 18 de la v8 empujaron al modelo a reescribir
 * con sus propias palabras, y se llevaron puesto el vocabulario del diario: la nota del Frigorífico
 * Tacuarembó dice "seguro de paro" siete veces, incluido el título, y la respuesta pasó a decir
 * "seguro de desempleo" (3 de 3 corridas el 15/9/2026, contra cuatro corridas seguidas bien antes).
 * Es correcto y suena importado: en Uruguay nadie lo llama así. La regla 19 acota la paráfrasis a
 * la estructura y deja quietos los términos.
 *
 * Historia previa (v8): el modelo abría el panorama describiendo el archivo ("las notas de la
 * sección X publicadas entre... incluyen:"). Con eso el sustento pasaba (0,73) y bloqueaba el
 * filtro de relevancia, que mide si el texto contesta la pregunta: un texto que habla de las notas
 * en vez de contar las noticias no la contesta. La regla 18 fija la apertura.
 *
 ...
 */
export const CANONICAL_SYSTEM_V9 = `Actuás como editor de El País (Uruguay). Respondés preguntas de lectores usando
...
19. Reformulá la estructura, no el vocabulario. Los nombres, los cargos, las cifras y los términos
    van como los escribe la nota: si dice "seguro de paro", es "seguro de paro" y no "seguro de
    desempleo"; si dice "intendencia", no es "municipio". Cuando la nota usa dos formas, quedate
    con la del título.
...
export const CANONICAL_STRICT_SUFFIX_V9 = `
