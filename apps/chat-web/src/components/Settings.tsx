...
import { STT_PROVIDER_OPTIONS, getSttProvider, setSttProvider, subscribeSttProvider } from '../lib/stt-provider';
...
export function Settings({ me, consent, onClose, onOpenGate, onChangeMode, onDelete }: Props) {
  ...
  const sttProvider = useSyncExternalStore(subscribeSttProvider, getSttProvider, getSttProvider);
    ...
    <Modal labelledBy={titleId} onClose={onClose} className="modal--settings" variant="drawer">
      ...
      <div className="modal-body settings-body">
        ...
        <section className="settings-section" aria-labelledby={`${titleId}-stt`}>
          ...
          <div className="segmented" role="radiogroup" aria-label="Quién reconoce tu voz">
            {STT_PROVIDER_OPTIONS.map((option) => (
              <button
                key={option.value}
                type="button"
                role="radio"
                aria-checked={sttProvider === option.value}
                className={sttProvider === option.value ? 'segmented-btn segmented-btn--active' : 'segmented-btn'}
                onClick={() => setSttProvider(option.value)}
              >
                {option.label}
              </button>
            ))}
          </div>
          <p className="settings-hint">{STT_PROVIDER_OPTIONS.find((option) => option.value === sttProvider)?.hint}</p>
          {sttProvider === 'navegador' ? (
